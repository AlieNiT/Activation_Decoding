...
class GreedySearchDecoderOnlyOutput(ModelOutput):
    ...
    # NOTE:
    adjust_scores: Optional[Tuple[torch.FloatTensor]] = None
...
class GenerationMixin:
    ...
    def generate(
        ...
        mj_threshold:Optional[float] = None,
        subject_key:Optional[int] = None,
        return_adjust_scores:Optional[bool] = None,
        ...
        **kwargs,
        ...
        if is_greedy_gen_mode and activation_decoding:
            ...
            return self.activation_greedy_decode(
                ...
                return_adjust_scores=return_adjust_scores,
        ...
        if is_greedy_gen_mode and activation_dola_decoding:
            ...
            # 11. run greedy search
            return self.activation_dola_greedy_decode(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
    ...
    def activation_greedy_decode(
        ...
        **model_kwargs,
        ...
        cross_attentions = () if (return_dict_in_generate and output_attentions) else None
        decoder_hidden_states = () if (return_dict_in_generate and output_hidden_states) else None
        # NOTE: add entropy or other metrics into the return dict
        adjust_scores = () if (return_dict_in_generate and output_scores) else None
        ...
        while True:
            ...
            dict_outputs, outputs = self(
                **model_inputs,
                ...
                early_exit_layers=early_exit_layers,
                info_layer=info_layer,
            ) # dict_outputs.keys() = [16, 18, 20, 22, 24, 26, 28, 30, 32] -> all layers' outputs
            ...
            if base_layer is not None:
                ...
                #####################################################################                
                ########################## ENTROPY STRATEGY #########################
                # we only consider the relation between every generated token and all the input sequence tokens
                # to reduce the inference computation cost, we compute every token's (in total 32000) relation to the input token only once, and use them for all the generated tokens 
                ...
                if len(before) == 0: # the token is the first generated token
                    ...
                    info_layer_score = before[0] # [num_token_in_question, len_token_lib] -> e.g. [62, 32000]
            ...
            if return_dict_in_generate:
                ...
                # NOTE: add entropy or other metrics into the return dict
                if return_adjust_scores:
                    adjust_scores += (adjust_score,)
