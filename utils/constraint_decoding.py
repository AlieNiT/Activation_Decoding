...
class ConstraintDecoding():
    ...
    def load_model(self, model_name):
        ...
        # model_name='/ssddata/shiqi/Explain_Jailbreak/honest_llama/validation/results_dump/llama2_chat_70B_seed_42_top_48_heads_alpha_15'
    ...
    def set_stop_words(self, stop_words):
        ...
        # self.stopping_criteria.append(list_stop_word_ids)

    def generate(self, input_text, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mature_layer=None, premature_layer=None, candidate_premature_layers=[], mode='baseline', verbose=False, remove_stop_words=False, relative_top=0.1, with_dola=False, return_outputs=True, return_adjust_scores=False, with_proper=False, **kwargs):
        with torch.no_grad():
            ...
            if mode == 'baseline':
                ...
                outputs = self.model.generate(input_ids, max_length=max_len, num_return_sequences=1,
                                    ...
                                    top_p=top_p, top_k=top_k, temperature=temperature, stopping_criteria=self.stopping_criteria, return_adjust_scores=return_adjust_scores,relative_top=relative_top, **kwargs)
            elif mode == 'dola':
                assert mature_layer is not None, "mature_layer must be specified"
    ...
    # def get_lm_scores_from_outputs(self, outputs, mode='baseline'):
    #     """
    #         Get the log-probability of the generated sequence from the model outputs
    #     """
        
    #     # final log probability of the generated sequence
    #     gen_len = len(outputs.scores)
    #     token_ids = outputs.sequences[0][-gen_len:]
    #     logit_scores = [outputs.scores[i][0, token_id] for (i, token_id) in enumerate(token_ids)]
    #     final_log_prob = sum(logit_scores)
        
    #     # final entropy of the generated sequence
    #     # pdb.set_trace()
    #     entropy_scores = [outputs.adjust_scores[i][0, token_id] for (i, token_id) in enumerate(token_ids)]
    #     final_entropy = sum(entropy_scores)
            
    #     # original log probability of the generated sequence
    #     original_log_prob = final_log_prob - sum(entropy_scores)
        
    #     adjust_score_dict = {'origin_log_prob': original_log_prob.item(), 'final_log_prob': final_log_prob.item(), 'entropy': final_entropy.item()}
        
    #     return adjust_score_dict
    ...
    #         elif mode == 'dola' or mode== 'with_dola':
    ...
    #                 if alpha!=0:
    #                     diff_logits = diff_logits + alpha*(-entropy)
    #                 log_probs = diff_logits[range(diff_logits.shape[0]), continue_ids].sum().item() 
    ...
    #                 relative_top_mask = self.get_relative_top_filter(final_logits, relative_top)
    #             mask = final_logits[0] < -1e3
    #             print(f'final_logits: {mask.shape}')
                
                
    #             if kwargs['decoding_strategy']=='entropy':
    #                 # info_layer_score=dict_outputs[kwargs['info_layer']][-1, :, :]
    #                 info_layer_score=dict_outputs[info_layer][-1, :, :]
    #                 print(f'info_layer_score: {info_layer_score.shape}')
    #                 index_nontop=torch.argwhere(mask).squeeze()
    #                 print(f'index_nontop: {index_nontop.shape}')
    #                 info_layer_probs = F.softmax(torch.t(info_layer_score), dim=1).unsqueeze(0) # info_layer_score: [num_token_in_question, len_token_lib] -> e.g. [250, 32000]
    #                 print(f'info_layer_probs: {info_layer_probs.shape}')
    #                 entropy = torch.distributions.Categorical(probs=info_layer_probs, validate_args = False).entropy()
    ...
    #                 output_attentions=False,
    #                 output_hidden_states=False,
    #                 early_exit_layers=[mature_layer],
    #                 info_layer=info_layer,
    #             )
    #             final_logits = dict_outputs[mature_layer][0, prefix_ids.shape[-1] - 1:-1].log_softmax(dim=-1)

    #             if relative_top > 0.0:
    #                 relative_top_mask = self.get_relative_top_filter(final_logits, relative_top)
    #             mask = final_logits[0] < -1e3
                
    #             if kwargs['decoding_strategy']=='entropy' and alpha != 0:
    #                 # info_layer_score=dict_outputs[kwargs['info_layer']][-1, :, :]
    #                 info_layer_score=dict_outputs[info_layer][-1, :, :]
    #                 index_nontop=torch.argwhere(mask).squeeze()
    #                 info_layer_probs = F.softmax(torch.t(info_layer_score), dim=1).unsqueeze(0) # info_layer_score: [num_token_in_question, len_token_lib] -> e.g. [250, 32000]
    ...
    #     return log_probs, (premature_layer_dist if mode == 'dola' or mode=='with_dola' else None)
